...
load_elf32mmu(paddr32_t addr, Elf32_Ehdr *hdr, Elf32_Phdr *phdr) {
		...
		case PT_LOAD:
			{
				...
				if(phdr->p_paddr != 0) {
					...
					elf_map(vaddr, (paddr_t)paddr, filesz & ~PAGEBITS, phdr->p_flags);
					...
					if(memsz) {
						...
						elf_map(vaddr + (filesz & ~PAGEBITS), (paddr_t)daddr, memsz, phdr->p_flags);
					}
				} else {
	...
	#error BOOTSTRAPS_RUN_ONE_TO_ONE must be defined
					...
					elf_map(vaddr, (paddr_t)daddr, memsz, phdr->p_flags);
#endif
				}
			}
			break;
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/startup/lib/elf.c $ $Rev: 759590 $")
#endif
