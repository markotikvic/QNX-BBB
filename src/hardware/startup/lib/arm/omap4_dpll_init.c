...
#define DPLL_DIV_M4_OFFSET                    0x18
#define DPLL_DIV_M5_OFFSET                    0x1c
#define DPLL_DIV_M6_OFFSET                    0x20
#define DPLL_DIV_M7_OFFSET                    0x24
...
int omap4_dpll_init(dpll_cfg_type * dpll_cfg)
{
  ...
  {
    // place DPLL in bypass mode and wait for that to take effect
    omap4_dpll_bypass(dpll_cfg->base_addr);
  ...
  omap4_dpll_set_output_divisor(dpll_cfg->base_addr + DPLL_DIV_M2_OFFSET, 
                                dpll_cfg->m2_div);
  omap4_dpll_set_output_divisor(dpll_cfg->base_addr + DPLL_DIV_M3_OFFSET, 
                                dpll_cfg->m3_div);
  omap4_dpll_set_output_divisor(dpll_cfg->base_addr + DPLL_DIV_M4_OFFSET, 
                                dpll_cfg->m4_div);
  omap4_dpll_set_output_divisor(dpll_cfg->base_addr + DPLL_DIV_M5_OFFSET, 
                                dpll_cfg->m5_div);
  omap4_dpll_set_output_divisor(dpll_cfg->base_addr + DPLL_DIV_M6_OFFSET, 
                                dpll_cfg->m6_div);
  omap4_dpll_set_output_divisor(dpll_cfg->base_addr + DPLL_DIV_M7_OFFSET, 
                                dpll_cfg->m7_div);
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/startup/lib/arm/omap4_dpll_init.c $ $Rev: 736706 $")
#endif
