...
extern uintptr_t 		arm_map(uintptr_t va, paddr_t pa, size_t sz, int flags);
extern void				arm_pdmap(uintptr_t va);
extern void				arm_scmap(uintptr_t va, paddr32_t pa, int flags);
...
extern uintptr_t        arm_lpae_map(uintptr_t va, paddr_t pa, size_t sz, unsigned flags);
extern void             arm_lpae_map_cpu(unsigned cpu, uintptr_t va, paddr_t pa, unsigned flags);
extern paddr_t          arm_lpae_elf_vaddr_to_paddr(uintptr_t vaddr);
extern void             arm_lpae_table_init(uintptr_t l2va, uintptr_t l3va);
extern void             arm_lpae_bmap(uintptr_t va, paddr_t pa, unsigned flags);
extern void             arm_lpae_bunmap(uintptr_t va);

extern unsigned			trap_vectors;
extern unsigned			mmu_cr_set;
extern unsigned			mmu_cr_clr;
extern int				cycles_per_loop;
extern paddr32_t		L1_paddr;
extern uintptr_t       	L1_vaddr;
extern paddr32_t		L2_paddr;
extern uintptr_t       	L2_vaddr;
extern unsigned         L2_size;
...
#define	ARM_MAP_SYSPAGE_RO	0xfffffffe	/* flags to arm_map() for read-only syspage/cpupage (LPAE only) */
#define ARM_MAP_NOEXEC	0x80000000		/* flags to arm_map() for I/O mem                               */
#define ARM_MAP_DEVICE  0x40000000		/* flags to arm_map() for I/O device                (LPAE only) */
#define ARM_MAP_STORDER 0x20000000		/* flags to arm_map() for strongly ordered memory   (LPAE only) */
#define ARM_MAP_NOCACHE 0x10000000		/* flags to arm_map() for non-cacheable memory      (LPAE only) */
...
extern const struct armv_cache	armv_cache_a7;
...
extern const struct armv_pte	armv_pte_lpae;
...
extern const struct armv_chip	armv_chip_a7_32;
extern const struct armv_chip	armv_chip_a7_lpae;
...
extern const struct armv_chip	armv_chip_a15;       // Generic chip
extern const struct armv_chip	armv_chip_a15_lpae;  // lpae version
extern const struct armv_chip	armv_chip_a15_32;    // non-lpae version
 ...
 * ------------------------------------------------------------------
 */
extern const struct armv_chip	*armv_detect_a7(void);

extern struct callout_rtn	page_flush_a7;
extern struct callout_rtn	page_flush_deferred_a7;
extern struct callout_rtn	cache_a7_i;
extern struct callout_rtn	cache_a7_d;
extern void					armv_setup_a7(struct cpuinfo_entry *, unsigned);
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/startup/lib/arm/cpu_startup.h $ $Rev: 761138 $")
#endif
