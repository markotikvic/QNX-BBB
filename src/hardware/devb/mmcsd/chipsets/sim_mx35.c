	...
	if (mask & (MMCINTR_BRR | MMCINTR_BWR)) {	// PIO
		...
		while (1) {
			if (sts & maskerrsts) {
				...
				if (sts & MMCINTR_DCE){
		    		intr |= MMC_ERR_DATA_CRC;
					if (1) {	//!mx35->sdcken){ // need to re-indentify the card to recover from data CRC error
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/devb/mmcsd/chipsets/sim_mx35.c $ $Rev: 736700 $")
#endif
