...
char *mmc_cmds[] = {	"MMC_GO_IDLE_STATE", "MMC_SEND_OP_COND",
						...
						"MMC_SET_DSR", "MMC_SLEEP_AWAKE", "MMC_SWITCH",
...
mmcsd_device_errata_t mmc_errata[] = {
		// device will discard data when AC12 is used
	{ "MAG2GA", 0x15, 0x0, 0x0, 0x6, DEV_ERRATA_ACMD12 },
		// device will discard data when AC12 is used
	{ "MAG4GA", 0x15, 0x0, 0x0, 0x6, DEV_ERRATA_ACMD12 },
	{ NULL, 0, 0, 0, 0 }
};

mmcsd_device_errata_t *mmcsd_device_errata( SIM_HBA *hba, mmcsd_device_errata_t *erratas )
{
	SIM_MMC_EXT				*ext;
	mmc_cid_t				*cid;
	mmcsd_device_errata_t	*errata;

	ext = (SIM_MMC_EXT *)hba->ext;
	cid = &ext->cid.mmc_cid;

	for( errata = erratas; errata->errata; errata++ ) {
		if( !strcmp( (char *)cid->pnm, errata->pnm ) &&
				cid->mid == errata->mid && cid->oid == errata->oid &&
				cid->fwr >= errata->prv_s && cid->fwr <= errata->prv_e ) {
			return( errata );
		}
	}
...
int mmcsd_reconcile_errata( SIM_HBA *hba, mmcsd_device_errata_t *errata )
{
	SIM_MMC_EXT	*ext;
	...
	if( ( errata->errata & DEV_ERRATA_ACMD12 ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: disabling ACMD12", __FUNCTION__ );
...
#define BL 16
#define AL BL/2*5+4
		sprintf( output, "%08ld:  %-*.*s\"%-*.*s\"", l, AL, AL, "  ", BL, BL, "  " );
	...
	if( ( status = mmc_sendcmd( hba, MMC_SWITCH, MMC_RSP_R1B, MMC_CMD_INTR, ( mode << 24 ) | ( index << 16 ) | ( value << 8 ) | cmdset ) ) == MMC_SUCCESS && flgs ) {
		...
		if( ( status = mmc_get_card_status( hba ) ) == MMC_SUCCESS ) {
			if( ( ext->mmc_resp[0] & MMC_SWITCH_ERROR ) ) {
				mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: idx %d, val %x, card status resp %x", __FUNCTION__, index, value, ext->mmc_resp[0] );
				status = MMC_ERROR;
			}
		}
	}
...
int mmc_send_cid( SIM_HBA *hba, uint32_t *cid )
{
	SIM_MMC_EXT		*ext;
	int				status;
	...
	if( ( status = mmc_sendcmd( hba, MMC_SEND_CID, MMC_RSP_R2, MMC_CMD_INTR, ext->rca << 16 ) ) == MMC_SUCCESS ) {
		status = mmc_wait_command( hba, MMC_TIME_DEFAULT );
	}

	cid[0] = ext->mmc_resp[0];
	cid[1] = ext->mmc_resp[1];
	cid[2] = ext->mmc_resp[2];
	cid[3] = ext->mmc_resp[3];

	return( status );
}

int mmc_send_csd( SIM_HBA *hba, uint32_t *csd )
{
	SIM_MMC_EXT		*ext;
	int				status;
	...
	if( ( status = mmc_sendcmd( hba, MMC_SEND_CSD, MMC_RSP_R2, MMC_CMD_INTR, ext->rca << 16 ) ) == MMC_SUCCESS ) {
		if( ( status = mmc_wait_command( hba, MMC_TIME_DEFAULT ) ) == MMC_SUCCESS ) {
			csd[0] = ext->mmc_resp[0];
			csd[1] = ext->mmc_resp[1];
			csd[2] = ext->mmc_resp[2];
			csd[3] = ext->mmc_resp[3];
		}
	}
...
int mmc_send_ext_csd( SIM_HBA *hba, uint8_t *csd )
{
	SIM_MMC_EXT		*ext;
	int				status;
	...
	ext->dir	= MMC_DIR_IN;
	ext->set_blksz( hba, MMC_DFLT_BLKSIZE );
	ext->setup_pio( hba, EXT_CSD_SIZE, MMC_DIR_IN );
	if( ( status = mmc_sendcmd( hba, MMC_SEND_EXT_CSD, MMC_RSP_R1, MMC_CMD_DATA_IN | MMC_CMD_DATA, 0 ) ) == MMC_SUCCESS ) {
		if( ( status = mmc_wait_command( hba, MMC_TIME_DEFAULT ) ) == MMC_SUCCESS ) {
			ext->pio_done( hba, (char *)csd, EXT_CSD_SIZE, MMC_DIR_IN );
		}
	}
...
int mmc_select_deselect_card( SIM_HBA *hba, int rca )
{
	int				status;

	status	= MMC_SUCCESS;

	if( ( status = mmc_sendcmd( hba, MMC_SEL_DES_CARD, rca ? MMC_RSP_R1: MMC_RSP_R1B, MMC_CMD_INTR, rca << 16 ) ) == MMC_SUCCESS ) {
		status = mmc_wait_command( hba, MMC_TIME_DEFAULT );
	}
...
int mmc_sleep_awake( SIM_HBA *hba, int flgs )
{
	SIM_MMC_EXT		*ext;
	int				status;
	uint32_t		arg;
	...
	if( ( status = mmc_sendcmd( hba, MMC_SLEEP_AWAKE, MMC_RSP_R1B, MMC_CMD_INTR, arg ) ) == MMC_SUCCESS ) {
		status = mmc_wait_command( hba, MMC_TIME_DEFAULT );
	}
...
int mmc_wait_card_status( SIM_HBA *hba, uint32_t mask, uint32_t val, uint32_t msec )
{
	SIM_MMC_EXT			*ext;
	int					cnt;
	int					stat;
	...
	stat	= MMC_FAILURE;
		...
		// fast poll for 10ms - 1ms intervals
	for( cnt = min( msec, 10 ); cnt; cnt-- ) {
		if( mmc_get_card_status( hba ) != MMC_SUCCESS ) {
			stat = MMC_FAILURE;
			break;
		}

		if( ( ext->mmc_resp[0] & mask ) == val ) {
			stat = MMC_SUCCESS;
			break;
		}

		if( ( ext->mmc_resp[0] & MMC_ERROR ) ) {
			stat = MMC_FAILURE;
			break;
		}
	...
	if( !cnt ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s:  mask %x, val %x, card status %x", __FUNCTION__, mask, val, ext->mmc_resp[0] );
	}

	return( stat );
}

int mmc_erase( SIM_HBA *hba, int partition, int flgs, uint64_t lba, int nlba )
{
	SIM_MMC_EXT		*ext;
	int				status;
	uint8_t			*ecsd;
	...
	wp_grp_size		= mmc_wp_grp_size( hba ) / 512;
#endif

	ext				= (SIM_MMC_EXT *)hba->ext;
	status			= MMC_SUCCESS;
	ecsd			= ext->extended_csd;
	tmult			= MMC_ERASE_TIMEOUT;
	
	erase_grp_size	= mmc_erase_grp_size( hba ) / 512;

#ifdef MMCSD_DEBUG
	mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: WP_GRP_SIZE %lld, lba %lld, nblks %d", __FUNCTION__, wp_grp_size, lba, nlba );
#endif

	if( mmc_set_partition( hba, partition ) == MMC_FAILURE ) {
		return( MMC_FAILURE );
	}

	if( ecsd[EXT_CSD_ERASE_GRP_DEF] ) {
		switch( flgs ) {
			case MMC_ERASE_NORM:
				tmult = MMC_ERASE_TIMEOUT * ecsd[EXT_CSD_ERASE_MULT];
				break;

			case MMC_ERASE_TRIM:
				tmult = MMC_ERASE_TIMEOUT * ecsd[EXT_CSD_TRIM_MULT];
				break;

			case MMC_ERASE_SECURE_TRIM:
				tmult = MMC_ERASE_TIMEOUT * ecsd[EXT_CSD_ERASE_MULT] * ecsd[EXT_CSD_SEC_TRIM_MULT];
				break;

			case MMC_ERASE_SECURE:
				tmult = MMC_ERASE_TIMEOUT * ecsd[EXT_CSD_ERASE_MULT] * ecsd[EXT_CSD_SEC_ERASE_MULT];
				break;
	...
	while( nlba ) {
		addr = ( ext->eflags & MMC_EFLAG_HC ) ? lba : ( lba * ext->mmc_blksize );
		if( ( status = mmc_sendcmd( hba, MMC_TAG_ERASE_GROUP_START, MMC_RSP_R1, MMC_CMD_INTR, addr ) ) == MMC_SUCCESS ) {
			status = mmc_wait_command( hba, MMC_TIME_DEFAULT );
		}

#if 1
//		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: lba %lld to %lld, timeout %d", __FUNCTION__, lba, lba + nlba - 1, tmult * ( nlba / erase_grp_size ) );
		timeout = tmult * ( nlba / erase_grp_size );
		lba		+= nlba;
		nlba	-= nlba;
#else
//		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: lba %lld to %lld, timeout %d", __FUNCTION__, lba, lba + min( nlba, wp_grp_size ) - 1, tmult * ( wp_grp_size / erase_grp_size ) );
		timeout = tmult * ( wp_grp_size / erase_grp_size );
		lba		+= min( nlba, wp_grp_size );
		nlba	-= min( nlba, wp_grp_size );
#endif
		lba--;
		addr = ( ext->eflags & MMC_EFLAG_HC ) ? lba : ( lba * ext->mmc_blksize );
		if( !status && ( status = mmc_sendcmd( hba, MMC_TAG_ERASE_GROUP_END, MMC_RSP_R1, MMC_CMD_INTR, addr ) ) == MMC_SUCCESS ) {
			status = mmc_wait_command( hba, MMC_TIME_DEFAULT );
		}

		// Set to R1, we will poll for card status
//		if( !status && ( status = mmc_sendcmd( hba, MMC_ERASE, MMC_RSP_R1B, MMC_CMD_INTR, flgs ) ) == MMC_SUCCESS ) {
		if( !status && ( status = mmc_sendcmd( hba, MMC_ERASE, MMC_RSP_R1, MMC_CMD_INTR, flgs ) ) == MMC_SUCCESS ) {
			status = mmc_wait_command( hba, MMC_TIME_INFINITY );
		}

		if( !status && ( status = mmc_wait_card_status( hba, MMC_READY_FOR_DATA, MMC_READY_FOR_DATA, timeout ) ) ) {
...
int mmc_write_protect( SIM_HBA *hba, int op, int ptype, int mode, uint32_t lba, uint32_t nlba )
{
	SIM_MMC_EXT		*ext;
	int				status;
	uint64_t		addr;
...
#ifdef MMCSD_DEBUG
	mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: WP_GRP_SIZE %lld, lba %d, nblks %d, mode %x", __FUNCTION__, wp_grp_size, lba, nlba, mode );
#endif

	if( op ) {
		switch( ptype ) {
			case MMC_PTYPE_USER:
				if( ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_USER_WP, mode ) ) == MMC_FAILURE ) {
					mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_user_wp", __FUNCTION__ );
					return( status );
				}
				break;

			case MMC_PTYPE_BOOT:
				if( ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_BOOT_WP, mode ) ) == MMC_FAILURE ) {
					mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_boot_wp", __FUNCTION__ );
					return( status );
				}
				break;
	...
	while( nlba ) {
		...
		if( ( status = mmc_sendcmd( hba, op ? MMC_SET_WRITE_PROT : MMC_CLR_WRITE_PROT,
				MMC_RSP_R1B, MMC_CMD_INTR, addr ) ) == MMC_SUCCESS ) {
			status = mmc_wait_command( hba, MMC_TIME_DEFAULT );
		}
...
int mmc_rpmb_rw_frame( SIM_HBA *hba, MMCSD_RPMB_FRAME *rpmb_frame, int partition, int dir, int reliable )
{
	SIM_MMC_EXT	*ext;
	int			cmd;
	int			flag;
	...
	ext = (SIM_MMC_EXT *) hba->ext;
	flag = MMC_CMD_DATA | MMC_CMD_DATA_MULTI;
	cmd = dir == MMC_DIR_IN ? MMC_READ_MULTIPLE_BLOCK : MMC_WRITE_MULTIPLE_BLOCK;
	...
	if( mmc_wait_card_ready( hba ) != MMC_SUCCESS ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: wait for card ready failed!", __FUNCTION__ );
		return( MMC_FAILURE );
	}

// mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: tid %d, lun %d, pact %d", __FUNCTION__, ext->nexus->cam_ch.cam_target_id, ext->nexus->cam_ch.cam_target_lun, ext->pactive );

	// switch partition to RPMB if necessary. CMD6
	if( mmc_set_partition( hba, partition ) == MMC_FAILURE ) {
		return( MMC_FAILURE );
	}

	// set block count. CMD23
	if( mmc_sendcmd( hba, MMC_SET_BLOCK_COUNT, MMC_RSP_R1, MMC_CMD_INTR, reliable ? 0x80000001 : 0x1 )
			!= MMC_SUCCESS ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: send set blk cnt cmd failed!", __FUNCTION__ );
		return( MMC_FAILURE );
	}

	if( mmc_wait_command( hba, MMC_TIME_DEFAULT ) != MMC_SUCCESS ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: wait for set blk cnt cmd failed!", __FUNCTION__ );
		return( MMC_FAILURE );
	}

	ext->setup_pio( hba, 512, dir );

	// multi block read/write. CMD18/CMD25
	if( mmc_sendcmd( hba, cmd, MMC_RSP_R1, flag, 0 ) != MMC_SUCCESS ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: send read/write cmd failed!", __FUNCTION__ );
		return( MMC_FAILURE );
	}

	if( mmc_wait_command( hba, MMC_TIME_DEFAULT ) != MMC_SUCCESS ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: wait for read/write cmd failed!", __FUNCTION__ );
		return( MMC_FAILURE );
	}

	ext->pio_done( hba, (char *)rpmb_frame, 512, dir );
	if( dir == MMC_DIR_OUT && mmc_wait_command( hba, MMC_TIME_DEFAULT ) != MMC_SUCCESS ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: wait for PIO done failed!", __FUNCTION__ );
		return( MMC_FAILURE );
	}
...
int mmc_send_rwcmd(SIM_HBA *hba, int nbytes, int flag)
{
	...
	// Data is expected, DON'T set MMC_CMD_INTR flag
	flag |= MMC_CMD_DATA;

#ifdef MMCSD_DEBUG
	mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: %s (%d), lba 0x%x, nblks 0x%x", __FUNCTION__, mmc_cmds[cmd], cmd, ( ext->eflags & MMC_EFLAG_HC ) ? ext->blkno : ( ext->blkno * ext->mmc_blksize ), nbytes / ext->mmc_blksize );
#endif
...
int mmc_dump_ext_csd( SIM_HBA *hba )
{
	uint8_t			csd[EXT_CSD_SIZE];
	int				status;

	InterruptUnmask( hba->cfg.IRQRegisters[0], hba->iid );
	status = mmc_send_ext_csd( hba, csd );
	InterruptMask( hba->cfg.IRQRegisters[0], hba->iid );
	mmc_dump( (char *)csd, 512 );
...
void mmc_parse_mmc_cid(SIM_HBA *hba, mmc_cid_t *cid, uint32_t *resp)
{
	...
	if( ( errata = mmcsd_device_errata( hba, mmc_errata ) ) ) {
		mmcsd_reconcile_errata( hba, errata );
	}
}
...
void mmc_parse_mmc_csd(SIM_HBA *hba, mmc_csd_t *csd, uint32_t *resp)
{
	...
	part					= &ext->targets[0].partitions[0];
...
static int mmcsd_mmc_config_buswidth(SIM_HBA *hba)
{
	...
	if( ext_csd_bit && ( ext->eflags & MMC_EFLAG_HS ) &&
			( ext->hccap & MMC_HCCAP_DDR50 ) && ( ext->dcap & MMC_DCAP_DDR ) ) {
		ext->eflags |= MMC_EFLAG_DDR;
		ext_csd_bit += 4;
	}
...
static int mmc_ident_switch_buswidth(SIM_HBA *hba, int intr)
{
	...
	if (hba->verbosity)
		slogf(_SLOGC_SIM_MMC, _SLOG_INFO, "MMC : Switch to bus width %dbit %ssuccessful", ext->bus_width,
				( ext->eflags & MMC_EFLAG_DDR ) ? "(DDR) " : "" );

	ext->set_blksz(hba, MMC_DFLT_BLKSIZE);	// block size, 512, every card should support

	if( ext->eflags & MMC_EFLAG_DDR ) {
			// MMC_SET_BLOCKLEN is invalid when we are in DDR mode.
...
#if 0
static void mmc_partition_config_rsvd( SIM_HBA *hba )
{
	SIM_MMC_EXT		*ext;
	MMC_PARTITION	*part;
	MMC_PARTITION	*rpart;
	...
	for( i = 0, nluns = ext->nluns + 1; i < nluns && i < MAX_LUN; part++, i++ ) {
		if( part->pflags & MMC_PFLAG_PART_RSV ) {
			if( part->pflags & MMC_PFLAG_PART_RSV_REMAP ) {
				rpart			= &ext->partitions[++ext->nluns];
				rpart->ptype	= part->ptype;
				rpart->pflags	= part->pflags;
				rpart->config	= part->config;
				rpart->nlba		= part->slba;
				rpart->slba		= 0;
				rpart->elba		= rpart->nlba - 1;
			}
			part->nlba	-= part->slba;
			part->elba	-= part->slba;
		}
...
int mmc_partition_config( SIM_HBA *hba, uint8_t *csd )
{
	...
	if( !( csd[EXT_CSD_PARTITIONING_SUP] & EXT_CSD_PS_EN ) || 
			!( ext->eflags & MMC_EFLAG_PARTITIONS ) ) {
		return( MMC_FAILURE );
	}
#else
	if( !( csd[EXT_CSD_PARTITIONING_SUP] & EXT_CSD_PS_EN ) ||
	...
	enh_size = (uint64_t)csd[EXT_CSD_MAX_ENH_SIZE_MULT + 0];
	enh_size |= (uint64_t)csd[EXT_CSD_MAX_ENH_SIZE_MULT + 1] << 8;
	enh_size |= (uint64_t)csd[EXT_CSD_MAX_ENH_SIZE_MULT + 2] << 16;
	mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: HC_WP_GRP_SZ %d, ERASE_GRP_SZ %d, ENH_SZ_MULT %d, SWITCH_TIME %dms, pw %x, pr %x", __FUNCTION__, csd[EXT_CSD_HC_WP_GRP_SIZE], csd[EXT_CSD_ERASE_GRP_SIZE], enh_size, csd[EXT_CSD_SWITCH_TIME] * 10, csd[235], csd[234] );

	part++;

		// boot area partition 1
	part->ptype		= MMC_PTYPE_BOOT;
	part->config	= pconf | 1;
	part->slba		= 0;
...
int mmc_create_partitions( SIM_HBA *hba, uint8_t *csd )
{
	...
	if( ( csd[EXT_CSD_WR_REL_PARAM] & EXT_CSD_WRP_WR_DATA_REL ) ) {
		if( mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_WR_REL_SET, EXT_CSD_WRS_USR | EXT_CSD_WRS_1 | EXT_CSD_WRS_2 | EXT_CSD_WRS_3 ) == MMC_FAILURE ) {
			mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_wr_rel_param", __FUNCTION__ );
		}
	}
	...
	wp_grp_size = (uint64_t )csd[EXT_CSD_HC_WP_GRP_SIZE ] *
		...
		// set partition 1 to 32MB/64MB (depends on wp_grp_size)
	part_size = max( 0x2000000 / wp_grp_size, 1 );
	if( ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_GP1_SIZE + 0, part_size & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_gp1_size 0", __FUNCTION__ );
	}

	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_GP1_SIZE + 1, ( part_size >> 8 ) & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_gp1_size 1", __FUNCTION__ );
	}

	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_GP1_SIZE + 2, ( part_size >> 16 ) & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_gp1_size 2", __FUNCTION__ );
	}

		// set partition 2 to 800MB
	part_size = max( 0x30000000 / wp_grp_size, 1 );
	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_GP2_SIZE + 0, part_size & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_gp2_size 0", __FUNCTION__ );
	}

	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_GP2_SIZE + 1, ( part_size >> 8 ) & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_gp2_size 1", __FUNCTION__ );
	}

	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_GP2_SIZE + 2, ( part_size >> 16 ) & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_gp2_size 2", __FUNCTION__ );
	}

		// set partition 3 to 512MB
	part_size = max( 0x20000000 / wp_grp_size, 1 );
	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_GP3_SIZE + 0, part_size & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_gp3_size 0", __FUNCTION__ );
	}

	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_GP3_SIZE + 1, ( part_size >> 8 ) & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_gp3_size 1", __FUNCTION__ );
	}

	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_GP3_SIZE + 2, ( part_size >> 16 ) & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_gp3_size 2", __FUNCTION__ );
	}

#if 0
		// set enhanced start address
	enh_addr = 0;
	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_ENH_START_ADDR + 0, enh_addr & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_enh_start_addr 0", __FUNCTION__ );
	}

	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_ENH_START_ADDR + 1, ( enh_addr >> 8 ) & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_enh_start_addr 1", __FUNCTION__ );
	}

	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_ENH_START_ADDR + 2, ( enh_addr >> 16 ) & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_enh_start_addr 2", __FUNCTION__ );
	}

	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_ENH_START_ADDR + 3, ( enh_addr >> 24 ) & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_enh_start_addr 3", __FUNCTION__ );
	}

		// set enhanced size 2MB to 64MB (1 * wp_grp_siz) 
	part_size = 1;
	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_ENH_SIZE_MULT + 0, part_size & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_enh_size_mult 0", __FUNCTION__ );
	}

	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_ENH_SIZE_MULT + 1, ( part_size >> 8 ) & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_enh_size_mult 1", __FUNCTION__ );
	}

	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_ENH_SIZE_MULT + 2, ( part_size >> 16 ) & 0xff ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_enh_size_mult 2", __FUNCTION__ );
	}

		// set enhanced attributes on USER, GP 1/2/3
	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_PARTITIONS_ATTR, EXT_CSD_PA_ENH_USR | EXT_CSD_PA_ENH_1 | EXT_CSD_PA_ENH_2 | EXT_CSD_PA_ENH_3 ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_partitions_attr", __FUNCTION__ );
	}
#else
		// set enhanced attributes on GP 1/2/3
	...
	if( !status && ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_PARTITION_SETTING, EXT_CSD_PS_CMP ) ) ) {
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_partition_setting", __FUNCTION__ );
	}
...
int mmc_erase_grp_config( SIM_HBA *hba, int hc )
{
	SIM_MMC_EXT		*ext;
	int				status;
	...
	InterruptUnmask( hba->cfg.IRQRegisters[0], hba->iid );
	if( ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_ERASE_GRP_DEF, hc ? EXT_CSD_EGD_EN : 0 ) ) == MMC_SUCCESS ) {
		ext->extended_csd[EXT_CSD_ERASE_GRP_DEF] = 1;
		if(mmc_wait_card_ready(hba)!=MMC_SUCCESS)
			return (CAM_FAILURE);
	}
	else {
...
#ifdef MMC_BKOPS_SUP
int mmc_bkops_config( SIM_HBA *hba )
{
	SIM_MMC_EXT		*ext;
	int				status;
	...
	if( !( ext->dcap & MMC_DCAP_BKOPS ) || !( ext->eflags & MMC_EFLAG_BKOPS ) ) {
		return( MMC_SUCCESS );
	}

	InterruptUnmask( hba->cfg.IRQRegisters[0], hba->iid );
	if( ( status = mmc_switch( hba, 1, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, EXT_CSD_BKOPS_EN, EXT_CSD_BKOPS_ENABLE ) ) ) {
		ext->eflags &= ~MMC_EFLAG_BKOPS;
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: switch ext_csd_bkops_en", __FUNCTION__ );
	}
	InterruptMask( hba->cfg.IRQRegisters[0], hba->iid );
...
static int mmc_ident_read_ext_csd(SIM_HBA *hba, int intr)
{
	...
	ext			= (SIM_MMC_EXT *)hba->ext;
	ext_csd		= &ext->csd.mmc_csd.ext_csd;
	raw_ext_csd	= ext->extended_csd;
	...
	if (ext->pio_done(hba, (char *)raw_ext_csd, EXT_CSD_SIZE, MMC_DIR_IN) != EXT_CSD_SIZE){
	...
	if( hba->verbosity > 3 ) {
		...
		mmc_dump( (char *)raw_ext_csd, EXT_CSD_SIZE );
	}

	ext_csd_struct = raw_ext_csd[EXT_CSD_REV];
	if (ext_csd_struct >= 2) {
		if( raw_ext_csd[EXT_CSD_SEC_FEATURE_SUPPORT] & EXT_CSD_SEC_GB_CL_EN ) {
			ext->dcap |= MMC_DCAP_TRIM;
		}

		if( raw_ext_csd[EXT_CSD_BKOPS_SUPPORTED] & EXT_CSD_BKOPS_SUP ) {
			ext->dcap |= MMC_DCAP_BKOPS;
			if( raw_ext_csd[EXT_CSD_BKOPS_EN] & EXT_CSD_BKOPS_ENABLE ) {
				// BKOPS enable is permanent (not in spec).
				// Note:  Firmware will not do them even if
				//        required to continue functioning,
				//        so we enable.
				ext->eflags |= MMC_EFLAG_BKOPS;
			}
		...
		ext_csd->sectors =
			raw_ext_csd[EXT_CSD_SEC_CNT + 0] << 0 |
			raw_ext_csd[EXT_CSD_SEC_CNT + 1] << 8 |
			raw_ext_csd[EXT_CSD_SEC_CNT + 2] << 16 |
			raw_ext_csd[EXT_CSD_SEC_CNT + 3] << 24;
	}

	// Needed for write protection
	ext->csd.mmc_csd.ext_csd.erase_grp_def = raw_ext_csd[EXT_CSD_ERASE_GRP_DEF];
	ext->csd.mmc_csd.ext_csd.hc_erase_group_size = raw_ext_csd[EXT_CSD_ERASE_GRP_SIZE];
	ext->csd.mmc_csd.ext_csd.hc_wp_grp_size = raw_ext_csd[EXT_CSD_HC_WP_GRP_SIZE];
	ext->csd.mmc_csd.ext_csd.user_wp = raw_ext_csd[EXT_CSD_USER_WP];

	if( hba->verbosity ) {
        slogf( _SLOGC_SIM_MMC, _SLOG_INFO, "MMC card : EXT_CSD structure: " );
        slogf( _SLOGC_SIM_MMC, _SLOG_INFO, " EXT_CSD_REV: %x", ext_csd_struct );
        slogf( _SLOGC_SIM_MMC, _SLOG_INFO, " Bus Width: %x", raw_ext_csd[EXT_CSD_BUS_WIDTH] );
        slogf( _SLOGC_SIM_MMC, _SLOG_INFO, " Card Type: %x", raw_ext_csd[EXT_CSD_CARD_TYPE] );
        slogf( _SLOGC_SIM_MMC, _SLOG_INFO, " SECTORS: %d", ext_csd->sectors );
    }

	switch( raw_ext_csd[EXT_CSD_CARD_TYPE] ) {
		case EXT_CSD_CARD_TYPE_200 | EXT_CSD_CARD_TYPE_DDR_52 | EXT_CSD_CARD_TYPE_52 | EXT_CSD_CARD_TYPE_26:
		...
		case EXT_CSD_CARD_TYPE_DDR_52 | EXT_CSD_CARD_TYPE_52 | EXT_CSD_CARD_TYPE_26:
			ext->dcap |= MMC_DCAP_DDR;
		case EXT_CSD_CARD_TYPE_52 | EXT_CSD_CARD_TYPE_26:
			ext_csd->hs_max_dtr = 52000000;
			break;
		case EXT_CSD_CARD_TYPE_26:
			ext_csd->hs_max_dtr = 26000000;
			break;
		default:
	...
	if( ( ext->csd.mmc_csd.mmc_prot >= CSD_VERSION_14 ) && ext_csd->sectors ) {
		ext->targets[0].partitions[0].nlba = ext_csd->sectors;
		ext->targets[0].partitions[0].elba = ext_csd->sectors - 1;
	}

	mmc_erase_grp_config( hba, raw_ext_csd[EXT_CSD_ERASE_GRP_SIZE] );

		// Needed for write protection
	ext_csd->erase_grp_def			= raw_ext_csd[EXT_CSD_ERASE_GRP_DEF];
	ext_csd->hc_erase_group_size	= raw_ext_csd[EXT_CSD_ERASE_GRP_SIZE];
	ext_csd->hc_wp_grp_size			= raw_ext_csd[EXT_CSD_HC_WP_GRP_SIZE];
	ext_csd->user_wp				= raw_ext_csd[EXT_CSD_USER_WP];

#ifdef MMC_BKOPS_SUP
	mmc_bkops_config( hba );
#endif
	mmc_partition_config( hba, raw_ext_csd );
//	mmc_partition_config_rsvd( hba );
//	mmc_create_partitions( hba, raw_ext_csd );

		// Activate high speed (if supported)
	if( ( ext_csd->hs_max_dtr != 0 ) && ( ext->hccap & MMC_HCCAP_HS ) ) {
		if( hba->verbosity )
			slogf( _SLOGC_SIM_MMC, _SLOG_INFO, "MMC: Switch to high speed mode." );

		ext->ident = MMCSD_IDENT_MMC_SWITCH_MODE_HS;

		return( mmc_sendcmd(hba, SD_SET_BUS_WIDTH, MMC_RSP_R1B, MMC_CMD_INTR,
				(0x3 << 24) | ( EXT_CSD_HS_TIMING << 16 ) | ( 1 << 8) | 0x1 ) );
		/* Activate wide bus (if supported). */
	}
...
static int mmc_ident_start(SIM_HBA *hba)
{
	...
	if (count == 0) {
		ext->targets[0].partitions[0].pflags &= ~MMC_PFLAG_WP;
		atomic_clr(&ext->eflags, MMC_EFLAG_PPL | MMC_EFLAG_HS | MMC_EFLAG_HC | MMC_EFLAG_DDR);
		memset( ext->extended_csd, 0, sizeof( ext->extended_csd ) );
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/devb/mmcsd/mmcsd.c $ $Rev: 736700 $")
#endif
